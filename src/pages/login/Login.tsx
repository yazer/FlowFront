...
import { getMethodWithToken } from "../../apis/ApiMethod";
import { login } from "../../apis/loginsignup";
import { GET_USER_PROFILE } from "../../apis/urls";
...
export function Login() {
  ...
  async function handleSubmit(values: any) {
    ...
    try {
      ...
      const userData = await getMethodWithToken(GET_USER_PROFILE, data.token);
      localStorage.setItem(
        "user_details",
        JSON.stringify(data?.user_details?.translations)
      );
      localStorage.setItem("profile_img", data?.user_details?.profile_img);
      window.location.href = userData?.redirect_url ?? "/process-list-v2";
    } catch (err: any) {
    ...
    <>
      ...
      <>
        ...
        <div className="flex flex-col mt-8">
          <p
            className="support_links underline"
