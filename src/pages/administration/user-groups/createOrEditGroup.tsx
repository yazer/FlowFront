...
import { CREATE_CATEGORY, CREATE_GROUP } from "../../../apis/urls";
...
function CreateOrEdit({
  ...
  const formik = useFormik({
    initialValues: { language: locale, name: "" },
    validationSchema,
    onSubmit: (values) => {
      const req = {
        name: values?.name ?? "",
  ...
  async function handleSubmit() {
    ...
    try {
      let reqBody: any = {
        name: form?.en?.name || "",
        translations: flattenLocalizedFields(form),
      };
      if (initialData) {
        await putMethod(CREATE_GROUP + initialData.id + "/", reqBody);
        toast.success(
          <FormattedMessage id="successUpdateCategory"></FormattedMessage>
        );
      } else {
        await postMethod(CREATE_GROUP, reqBody);
        toast.success(
...
type FlattenedOutput = {
  name: {
    [lang: string]: string;
  };
};

function flattenLocalizedFields(input: LocalizedInput): FlattenedOutput {
  const result: FlattenedOutput = { name: {} };

  for (const [lang, value] of Object.entries(input)) {
    if (value && typeof value.name === "string") {
      result.name[lang] = value.name;
    }
  }
